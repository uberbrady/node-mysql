...
function QueuedClientPool(config) {
	...
	for(i=0;i<this.numconns;i++) {
		this.reconnect(this.connections[i]);
	}
	this._queue=[];
}
...
QueuedClientPool.prototype.reconnect = function (conn) 
{
	console.warn("_errorhandler has reported one of the connections has thrown an error");
	conn=new Client();
	hashish.update(this.connections[i],config || {});
	conn.on('connected',this._runqueue);
	conn.on('error',function() {self._errorhandler(this.connections[i]);});
/*	var i;
	var activeconns=0;
	for(i=0;i<this.numconns;i++) {
		if(!this.connections[i].connected) {
			if(!this.connections[i].current) {
				console.warn("Trying to reconnect for connection #",i);
				this.connections[i].connect();
			} else {
				console.warn("Connection #",i," is already trying to be connected...");
			}
		} else {
			activeconns++;
		}

...
Client.prototype.connect = function() {
  ...
  var self = this;
	var current = {sql: "Connecting To Database...",fn: function () {console.warn("I dunno. something. whatever.")}};
  socket
    ...
    .on('end', function() {
      ...
      //self._connect();
		...
		.on('close',function(why) {
			console.warn("CLOSE IS HAPPENING: ",why," current is: ",self.current," ending is: ",self.ending); 
			if(!self.ending) {
	...
	socket.setTimeout(/* self.retry* */1000,function() {console.warn("Connect timeout thing?");self.emit('error',new Error("Time Out!"));});
	...
	this.on('error',function (err) {
		...
		if(self._socket) {
			self._socket.end(function() {console.warn("Socket was ended!")});
		}

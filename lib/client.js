    ...
    .on('end', function() {
      ...
      //self._connect();
		...
		.on('close',function(why) {
			console.warn("CLOSE IS HAPPENING: ",why); 
			if(this.current) {
				console.warn("AND we havea  current query!!!!");
				this.emit('error',new Error('socket is closing while a query is running.'));
			}
			//self._socket=null; //is this necessary? This yanks the socket from existence - don't we want to ease it on out?
		})
    .connect(this.port, this.host);
	socket.on('connect',function () {console.warn("Socket is connected - but you ain't.")});
...
Client.prototype.query = function(sql, params, cb) {
  ...
  this._execute(function () {
    ...
    if(self.write(packet)) {
			//good, you wrote the packet without error
		} else {
			query.emit('error',new Error("Could not write to socket"));
		}
  }, query);
...
Client.prototype._handlePacket = function(packet) {
  ...
  if (!this.connected) {
    if (packet.type != Parser.ERROR_PACKET) { //should this be == Parser.OK_PACKET ?
      this.connected = true;
			this.current=null;//and we're finished connecting, too
  ...
  if (type != Parser.ERROR_PACKET) {
	...
	//or can this not run until the delegate has run?
  this.current=null; //we're done? Right? One packet = one answer, right?
};
